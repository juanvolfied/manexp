...
<div>    
    ...
    <div style='padding-left:30px;'><br><table align='center'><tr><td><b>INVENTARIO DE TRANSFERENCIA DE CARPETAS FISCALES (F.P.P.C.)</b></td></tr></table>
    ...
    <table id="scanned-list" class="table table-striped table-sm" width="100%" border=1 style="border-collapse: collapse;">
        <thead class="thead-dark">
            ...
            <th width=13% align="center" rowspan=2>CARPETA FISCAL</th>			      
        ...
        @foreach ($regdet as $detalle)
            <tr>
            <td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $loop->iteration }}</td>
            <!--<td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $detalle->codbarras }}</td>-->
            ...
            <!--<td style="font-size:12px; padding: 5px 10px !important;">{{ $detalle->id_dependencia }}</td>
            <td style="font-size:12px; padding: 5px 10px !important;">{{ $detalle->ano_expediente }}</td>
            <td style="font-size:12px; padding: 5px 10px !important;">{{ $detalle->nro_expediente }}</td>
            <td style="font-size:12px; padding: 5px 10px !important;">{{ $detalle->id_tipo }}</td>-->                        
            <td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $detalle->nro_folios }}</td>
            <td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $detalle->agraviado }}</td>
            <td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $detalle->imputado }}</td>
            <td style='padding-top:10px; padding-bottom:10px;' align="center">{{ $detalle->desc_delito }}</td>
            </tr>
        @endforeach
    </tbody>
    </table>
