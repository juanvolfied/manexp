                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-md-2 d-flex align-items-end">
                            <a href="#" onclick="buscadatos(event)" class="btn btn-primary w-100">
                                <i class="fas fa-arrow-right me-1"></i> Consultar</a>
                        </div>
...
function mostrarDetalle(anio, mes, codigo) {
    ...
    async function imprimirpdf() { 
        const escritos = @json($escritosporcf);
        const carpfisc = "{{ $carpetafiscal }}"; 

        const loader = document.getElementById('loader');
        loader.style.display = 'block'; 

        try {
            const response = await fetch("{{ route('mesapartes.imprimirescritosporcf') }}", {
                method: 'POST',
                headers: {
